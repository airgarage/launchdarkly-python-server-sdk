...
class CacheConfig:
    ...
    def __init__(
        self, expiration: float = DEFAULT_EXPIRATION, capacity: int = DEFAULT_CAPACITY
    ):
...
class InMemoryFeatureStore(FeatureStore, DiagnosticDescription):
    ...
    def init(self, all_data):
        ...
        try:
            ...
            for k in all_data:
                log.debug(
                    "Initialized '%s' store with %d items",
                    k.namespace,
                    len(all_data[k]),
                )
        finally:
    ...
    def _sort_collection(kind, input):
        ...
        while len(remaining_items) > 0:
            ...
            for key, item in remaining_items.items():
                _FeatureStoreDataSetSorter._add_with_dependencies_first(
                    item, dependency_fn, remaining_items, items_out
                )
                break
        return items_out
    ...
    def _add_with_dependencies_first(item, dependency_fn, remaining_items, items_out):
        ...
        for dep_key in dependency_fn(item):
            ...
            if dep_item is not None:
                _FeatureStoreDataSetSorter._add_with_dependencies_first(
                    dep_item, dependency_fn, remaining_items, items_out
                )
        items_out[key] = item
